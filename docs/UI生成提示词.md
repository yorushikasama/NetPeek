...
> ---
>
> **2026-09-15 增补：材质层（不透明度 / 压暗 / 对比度口径）已被玻璃契约取代，本文不再是它的来源。**
>
> 本文 3.2 与 3.7 写的是**固定材质**规格：岛屿不透明度下限 0.82 / 默认 0.88、全局压暗纯黑 0.30，
> 并按这几个数算出一张 WCAG 2.1 对比度表。实现现在走的是另一套模型，两处都不再吻合：
>
> | 本文 | 实现（`ui/theme.js`） | 为什么变 |
> | --- | --- | --- |
> | 面板不透明度下限 0.82、默认 0.88 | 下限 `MIN_PANEL_OP` 0.30、默认 0.45 | 0.82 的面板等于「几乎实底」，用户两轮反馈都是「不透明度拉到最低仍看不见底图」。可读性不再靠堆漆层，改由**面板级透镜**（`backdrop-filter` 把面板身后那块底图归一化到一条明度带）承担 |
> | 全局压暗纯黑 0.30 是可读性的一部分 | 压暗只影响面板**缝隙**；面板身后由透镜反解，压暗被透镜抵消 | 透镜按「veil 之后的图」反解，veil 做多少它就抵消多少（实测纱 0.20→0.60 在面板下的输出差 <0.001） |
> | WCAG 2.1 亮度比 4.5:1 | APCA Lc（正文 ≥60、其余字色 ≥45、弱字阶 ≥30） | 面板被底图稀释成中调后，WCAG 的比值会给出「正文 6.6 达标」这种与人眼相反的结论 |
>
> 所以：**3.2 的数值表、3.7 第 4 条（禁止不透明度低于 0.82）与 1.3 里「下限从 0.75 提到 0.82」这一行，
> 都只作为设计史保留，不是当前规格**。材质层的唯一来源是 `ui/theme.js` 的玻璃契约
> （`glassContract` / `lensBand` / `glassHardenTokens`），判据由 `scripts/ui-tests/theme.test.mjs` 锁住、
> 由 `scripts/preview-v2.mjs` 的像素审计做地面真值。本文第 1–3 节的**结构、密度、语义色、字阶层级**仍然有效。
>
> 三模式主题系统（模式 1 取色 / 模式 2 AI / 模式 3 定制）已退役，现在是皮肤模型
> （朴素 / 浅色 / 琥珀 / 跟随背景图 + 自定义），见 [开发进度.md](开发进度.md) §46。
...
> **2026-09-14 修订**：列已增至六列 —— 其后陆续加了「对端」（国旗 / 保留地址图标 +
> IP:端口 + 服务名）与「近 24 小时」（历史库 `minute_stats` 的下载 + 上传合计，可排序，
> 无记录显示 `—`）。上面「只有四列」指的是 v2 定稿当时的取舍（活动 / 占比 / 累计 / 重传
> 四列下岛），仍成立；新增的两列都带明确口径与单位，不占岛外空间。
> 六列在 940×620 下的宽度预算见 `docs/开发进度.md` 33.5。

行状态：悬浮 → 底色 `--surface-hi`；选中 → 底色 `--surface-hi` + 应用名字重 600 + 检查栏跟随，
不加行首色条。

排序点列头，三态循环（`docs/开发进度.md` §36）：默认态表格按「下载」降序呈现但**不标箭头**；
点列头 → 该列首选方向（文本列升序、其余五列降序）→ 再点反向 → 第三次取消排序回默认态。
排序列的列头文字提亮到 `--text`，箭头随 `currentColor` 一起亮 —— 不用 `--down`/`--up`，
排序是列表状态不是数据（「上传」列戴上下载色是错的）。排序生效时表格卡片头露出
**「恢复默认排序」图标钮**（`.icon-btn.sort-reset`，与搜索框同高 28px，默认态 `hidden`），
一步复位行序 / 箭头 / 按钮三样。
...
- 下方 172：双色分组柱状图（ECharts，2026-09-06 起）。y 轴三档占 52，绘图区 1068。
  x 轴只标两端日期和每个月初那一根。
- **聚合粒度**（2026-09-06 修订，替代「近 90 天按周」）：预设 7/30/90 天 = 本地日桶
  （ECharts 下 90 根日柱 ~11px 仍可读可点，保留日粒度比周粒度有用）；自定义 ≤48h = 小时桶、
  >90 天 = 周桶。非日桶时胶囊右侧补一行 11px 聚合提示（「按小时聚合」/「按周聚合」）。
- **周桶锚点**：以区间起点所在周的**本地周一零点**为锚（前端 Date.getDay 算出传给
  history_range，SQL `((ts-?3)/604800)*604800+?3`）。UTC 周（1970 周四对齐）在 UTC+8 下
  会从周四 08:00 开始、首日只有 8 小时，标签对不上。
...
**外观屏（数据岛）**，双列 + 全宽高级条（2026-09-06 修订：原「自上而下单列」
在 250px 高的数据岛里要滚 629px 的内容，常态就该零滚动）：

1. **壁纸条**（左列）：内置壁纸 ×3（缩略图）＋「＋自选」＋「无」，3+2 两行。
   选中项琥珀描边 + 角标 ✓。换壁纸立即重新取色（来源=取色）；「无」同时关掉留白区的
   邀请提示（用户已做决定，不再问）。状态行文案带当前壁纸名（「当前：熔金暮色（内置）」）。
2. **材质**（右列）：三根滑杆竖排（岛屿不透明度 0.82–1.00 默认 0.88 / 全局压暗 0.20–0.60
   默认 0.30 / 背景模糊 0–40 默认 24；范围是 3.2 的实测结果）。滑杆头 label/value 紧靠
   （space-between 会把值推到列右缘，视觉上粘到下一行的标签）。滑杆**任何时候只改材质**，
   不重生成令牌；无背景时三根滑杆禁用并显示说明（无背景时它们本来就没有效果）。
3. 「无背景时跟随系统深浅色」开关（仅无背景时可见；浅色系统用浅色预设），材质列底部。
4. **高级折叠区**（details，全宽压在下栏，默认收起）：手动微调语义色开关（开启后
   10 色板任何来源可编辑，下载/上传保持语义锁定；色板 5 列 × 2 行，全宽下比 3 列省一半
   纵向）＋三个预设按钮（直接覆盖当前语义色）＋AI 自适应（接口地址 / API Key /
   模型三项并排、授权勾选、生成按钮）。AI 同图按缩图像素哈希缓存复用，只上传 ≤512px
   缩略图，20s 超时自动回退离线取色。**展开即编辑态切换**（2026-09-06 修订）：展开内容
   ~330px，数据岛净高只有 ~320px，原地展开必然把壁纸/材质双列顶出可视区——所以展开时
   双列隐藏、编辑器独占整岛（展开内容限高内部滚动），收起恢复双列并复位滚动。

**外观屏（检查栏）**：当前主题卡（双色 chip + 名称 + 来源与材质摘要）｜对比度校验结果
（「✓ 全部达标」或「N 项已校正」，校验即引擎里的 WCAG 4.5:1 自动校正，这里只是把它变成
用户可见的质量信号）｜保存当前为主题｜主题列表（应用 ✓ / 内联重命名 ✎ / 删除 🗑；行操作悬停才显示，当前项的 ✓ 是选中态信号恒显；
...
- **无背景图**时岛屿退回不透明 `--surface` 并关掉 `backdrop-filter`（没有底图，模糊白耗 GPU）；
  留白区保持空白、不放任何引导块 —— 「不设背景图」是正当稳态，常驻推广位违背 §2.1，
  背景图入口收敛到外观屏（2026-09-12 评估后移除原「选择背景图」引导块）；
  无底图时的虚空感由 backdrop 的暗场渐变承担，不靠引导块填补。
...
> **发光预算（2026-09-12 补）**：上面这句「留 8px」和下面那条「外发光 `0 0 20px`」
> 是自相矛盾的 —— 20 > 8，照规格画出来必然被窗口裁成直线；而圆形的光被方形窗口
> 裁出来，就是四个角什么都没有、四条边中段各一条亮线，读作「球外面套了个方框」。
> **硬约束：任何光的最远半径 ≤ 窗口半宽 54（球半径 46 + 8）。** 于是球自己的外发光
> 只能取 6px（46+6=52）；大范围光晕交给 `.orb-halo` 的径向渐变（外扩 5px、半径 51，
> 是唯一有铺开余量的层），且它的渐变末档必须归零，不能在元素边界上留着半透明色。
> 同理，面板形态的 panel 与窗口之间要留 12px，两层 box-shadow 的扩散之和不得超过它。

- 底：`--surface` 0.88 + 1px `--edge` 亮边 + 外发光 `0 0 6px var(--glow-warm)`
  （预算见上）。**球体要有 3D 感**：`.orb::before` 用暖色径向渐变叠出球面明暗
  （左上受光、右下背光、底部一道环境反弹光），`.orb::after` 在受光方向给一个
  小而锐的固定镜面高光。这两层是**材质**，不随能量 `--e` 变化 —— 静默时球也得是球。
  去掉现在的 `radial-gradient(#2b3542 → #14181f)` 冷色球体渐变 —— 那是个塑料按钮的质感，
  和主界面的暖色岛屿不是一套材质（补球面光照同样只能用暖色，不能退回冷色）。
...
**迷你窗** 面板 320×300（**窗口 344×324**，四周各 12px 留给投影），圆角 16（现在是 10）：
...
> **【已被取代，2026-09-15】** 本小节以下的数值（不透明度 0.82/0.88、纯黑压暗 0.30、那张 WCAG 对比度表）
> 是固定材质时代的规格，实现已换成玻璃契约（下限 0.30 / 默认 0.45 + 面板级透镜 + APCA）。
> 保留它是因为**推理过程仍然成立**：0.78 会让 `--up`/`--error` 在底图亮部不合格，
> 这条约束正是后来「必须给面板身后那块背景做归一化」的起点。详见文首增补与 [开发进度.md](开发进度.md) §46。
...
**右键菜单不用系统原生菜单，也不用浏览器菜单。** 弹出的必须是应用自己的浮层：
底漆 `--panel-hi` × `--paint-op`、阴影随 `--ui-opacity` 淡出、圆角 `--r-m`，
与下拉菜单（`.sel-pop`）完全同口径 —— 右键菜单和下拉菜单是同一类东西，
半透明界面下两者必须一致地「浮」。原生菜单的配色来自系统，换肤后它不跟着走，
托盘菜单就是这么栽的（开发进度 §34.5、§35）。

菜单里给的是**动作**而不是设置入口：默认项就是「复制」，因为在这套界面里
「把 IP / 路径拿出去用」是右键最想干的事。左列写动作、右列用等宽小字写「将要复制到的内容」，
让人一眼确认复制的是什么（真值是完整的，右列只是预览，超长截断）。
只有「选中文本 / 单元格声明了复制值 / 落在表格行上」三种上下文，才弹菜单；
落在空白处不弹 —— 空菜单比没有菜单更糟。

**右键菜单与可选中文本是一套。** 开放 `user-select: text` 的元素要和菜单能复制的东西对上：
表格数据格、检查栏的路径 / 详情 / 字段、今日合计、历史排行与合计数。
`body` 的 `user-select: none` 保留（拖过一行不该选中半屏控件文字），
按元素点名放行；I 形光标只给真正的「值」（对端列、检查栏那几处），
表格行仍是 pointer —— 满屏 I 形光标会把「点这一行看详情」这条信息一起抹掉。
...
#### 3.6.1 唯一的例外：启动动效（2026-09-14 新增）

上面「没有入场编排」管的是**主界面**：窗口一露出来就应该是最终状态。**启动动效在它之前**，
是启动层（`ui/boot.css` + `ui/boot.js`）自己的一屏，主界面仍然不做入场编排。两者不冲突。

它同时对 3.7 的禁止项开了两个口子，都是有边界的、写在这里以免被当成违规：

| 禁止项 | 启动动效的口径 |
|---|---|
| 5. 琥珀只表示「下载」 | 启动页是全应用唯一允许出**品牌色**的位置（与应用图标、安装包、任务栏图标同一套）。这条约束管的是界面内的控件与选中态，品牌露出不在其中。应用内顶栏的品牌标仍是中性单色（`currentColor`），两者是同一套品牌系统的两个用法。要完全合规，`NetPeekBoot.setPreset('chrome')` 切中性反白预设 |
| 7. 不做跟着数据走的动效 | 启动动效与任何数据无关，是纯揭示，且**每次页面加载只播一次**；隐藏到托盘再唤出不重播（高频操作，每次重播会变成噪音） |

生成主界面之外的任何东西（营销图、官网、商店页）如果要用这个启动动效，
按 `assets/boot-motion/motion_spec.md` 的实测参数来，不要另配一套时间线。


### 3.7 这一版的禁止项

1. 在留白区放任何 UI —— 无条件禁止，无底图态也不放（原无底图引导块已移除，见 2.8）。
2. 在半透明岛屿上用深色描边（现有实现的 `--border: #2a2f38` 那一类）—— 底图亮部实测只剩 1.12 对比。
3. 满格横向分隔线的表格。
4. ~~岛屿不透明度低于 0.82，或全局压暗低于 0.20。~~
   **【已被取代，2026-09-15】** 现行实现的滑杆下限就是 0.30、默认 0.45，这一条按字面读是被
   *有意*违反的。它当年要挡的风险（面板太透 → 底图亮部把文字吃掉）现在由**玻璃契约**挡：
   面板级透镜先把身后那块背景归一化进一条明度带，带端按「正文 APCA ≥ 60 + 合成面不滑进中调」
   反解，其余字色由加固层对着最不利合成面逐键推到达标。换句话说约束没有放松，是换了实现位置 ——
   从「把面板做厚」换成「把面板身后那块背景做干净」。判据见 `theme.js` 的 `lensBand` /
   `glassHardenTokens`，回归见 `scripts/ui-tests/theme.test.mjs`。
5. 用琥珀表示除「下载」以外的任何东西：服务状态点、警告、当前选中都不许借用。
   （唯一例外：启动动效的品牌露出，见 3.6.1。）
