...
上一版这两屏是**覆盖层**：`rgba(0,0,0,0.45)` 铺满全窗、右侧滑出 380 面板（`styles.css:472-481`）。
调配色的面板把被调的界面压暗 45% —— 在一个失真的画面上调颜色。
这一版改成**屏**：从导航岛进入，占用与实时屏完全相同的岛位置，不盖住任何东西。

**外观屏**

- 检查栏岛位置 = 模式三选一（标准离线取色 / AI 自适应 / 定制化主题）+ 背景图
  （348×120 缩略图、「选择图片…」「无背景」两个按钮、当前文件名 12px 单行截断）。
- 数据岛位置 = 三列等宽（每列 362，间距 16）：**微调**（三个滑块）｜**语义色**（10 个色块，
  下载与上传两个带「语义锁定」提示）｜**主题列表**（名称输入 + 保存 + 已存主题列表，当前项亮边）。
- 留白区 = **实时预览本身**。顶栏岛、导航岛、数据岛都还在画面里，改不透明度或语义色时它们当场变，
  不需要造一张假的预览卡片。这是把外观做成屏而不是覆盖层最大的收益。

三个滑块的范围是实测结果，不是手感（推导见 3.2）：

| 滑块 | 范围 | 默认 | 依据 |
| --- | --- | --- | --- |
| 岛屿不透明度 | 0.82 – 1.00 | 0.88 | 0.82 是底图纯白区域下 `--up` 仍过 4.5:1 的下限（实测 4.78） |
| 全局压暗 | 0.20 – 0.60 | 0.30 | 低于 0.20 时连 0.82 的岛屿也压不住高亮底图 |
| 背景模糊 | 0 – 40 | 24 | 允许 0（有些底图本身干净），但岛内文字会压在细节上 |

上一版下限 0.30（`index.html:138`）实测在纯白底图上把次要文字压到 1.77:1 —— 可以拖到读不了。
**下限由令牌算出来，不留不可读的余地**；想要更透，先把全局压暗拉高。

AI 模式的三个字段（接口地址 / API Key / 模型）沿用现有实现，API Key 用密码框且不回显；
**勾选同意后才允许上传缩略图**，未勾选时「生成并应用」是 disabled 态，而不是点了报错。

**设置屏**

- 检查栏岛位置 = 常规：速率显示单位（自动 / 固定 KB / MB / GB）、登录时自动启动、
  启动时最小化到托盘、小窗默认形态（能量球 / 迷你窗）。
- 数据岛位置 = 三列：**历史数据**（保留周期、库占用、清空历史）｜**采集服务**（运行状态、
  ETW 丢事件计数、重启服务）｜**关于**（版本、日志目录链接、开源许可）。
- 「清空历史」和「重启服务」是破坏性动作，**原地二次确认，不弹模态框**：按钮就地变成
  「确认清空 3.2 GB？」+「取消」，5 秒无操作自动收回。重启服务需要管理员权限，按钮带盾牌图标并注明。
