...
(function () {
  ...
  // 能量填充的刻度：**档位锚点 + 档内对数**，而不是「取第一个容得下的档做分母」。
  //
  // 三版演进，前一版的问题都记着，免得再来回翻：
  // ①「近 60 秒峰值」做分母 —— 浮动刻度：填充走到 80% 时读不出是 8 MB/s 还是
  //   80 KB/s，下载一停峰值塌下去、同样的速率过一会儿水位反而更高。量尺会自己
  //   改刻度，量出来的数就没有意义。作废。
  // ② 固定档位做分母（bps / 档位）—— 刻度固定了，但**跨档时水位会掉一大截**：
  //   刚过 1 MB/s 时是 1.000，过一点就变成 0.080，跳幅 0.87–0.92 个满量程。
  //   实测（2026-09-21）：1.10 MB/s 落进 12.5 MB/s 档，水位只有 8.8%，
  //   而 0.99 MB/s 是 99% —— 速率涨了 11%，水位掉了 90%。作废。
  // ③ 现在这版：档位仍是「带宽量级」的锚点（1 MB/s ≈ 百兆宽带的零头，
  //   12.5 MB/s ≈ 百兆满速，125 MB/s ≈ 千兆满速），但每个档位对应一个**固定的
  //   水位高度**（ANCHORS），档内按对数插值。于是：
  //   · 单调：速率涨，水位只涨不跌（同一速率永远对应同一水位）；
  //   · 连续：档位边界处水位严丝合缝（实测跳幅 0.0000）；
  //   · 仍可读：看到水位 0.52 就知道是 12.5 MB/s（百兆满速）那一档。
  //   对数而不是线性：宽带速率本来就是跨量级的（KB/s → MB/s → GB/s），
  //   线性映射会把日常那几百 KB/s ~ 几十 MB/s 全挤在球底一小段里。
  const TIERS = [128e3, 1e6, 12.5e6, 125e6, 1.25e9];
  // 每个档位对应的水位（0–1）。取近似等差的五档：档位之间视觉上均匀，
  // 最后一档落在满格。第一档给 0.10 而不是 0 —— 128 KB/s 已经是不小的流量，
  // 压在球底一条线里会读成「几乎没有」。
  const ANCHORS = [0.10, 0.28, 0.52, 0.76, 1.0];
  ...
  /// 速率 → 水位（0–1）。分段对数：TIERS 是锚点、ANCHORS 是锚点水位。
  ///
  /// 纯函数、无状态：同一个速率永远给出同一个水位，不依赖「上一次是多少」。
  /// 这一点是刻意的 —— 早期版本试过带回差的滞回（防止水位在边界抖动），
  /// 但那让水位的含义变成「取决于你从哪个方向来」，一个读数就不该有这种性质。
  /// 现在档位边界本来就连续（跳幅 0），滞回没有存在的必要。
  ///
  /// 边界处理：
  /// · bps ≤ 0 → 0（球空）；
  /// · 低于第一档（128 KB/s）→ 从 0 线性升到第一锚点。这一段不做对数：
  ///   越接近 0，log 的斜率越陡，几 KB/s 的抖动会让水位乱跳（后台心跳流量
  ///   就是这量级），而那点流量本来也不该在水位上显出来；
  /// · 超过最末档（1.25 GB/s，万兆满速）→ 满格。
  function levelOf(bps) {
    if (!(bps > 0)) return 0;
    if (bps <= TIERS[0]) {
      return ANCHORS[0] * (bps / TIERS[0]);
    }
    for (let i = 0; i < TIERS.length - 1; i++) {
      const lo = TIERS[i];
      const hi = TIERS[i + 1];
      if (bps <= hi) {
        // 档内按对数插值：两个锚点之间，频率比取 log 才是等距的
        const f = Math.log(bps / lo) / Math.log(hi / lo);
        return ANCHORS[i] + f * (ANCHORS[i + 1] - ANCHORS[i]);
      }
    }
    return 1;
  }

  /// 水位写进 CSS 变量（--level）：液柱取它当**高度**，液面波形取它当
  /// **bottom 基准**（两者都要贴着同一条水位线走）。
  /// 所以写在球本体（两者的共同祖先）上，而不是液柱自己身上 ——
  /// 2026-09-21 加波形时踩过这个坑：变量挂在液柱上，波形层是它的**兄弟**
  /// 而非后代，继承不到，calc(var(--level, 0%)) 静默退化成 0，
  /// 波面直接沉到球底。CSS 自定义属性取不到值不报错、只是回落，
  /// 外观上很难往回追（是靠量 getBoundingClientRect 才定位到的）。
  /// 挂在球上还有个好处：以后 .orb-level 内部再怎么重构，两个消费者
  /// 都在同一棵子树里，不会再出现「一个读得到一个读不到」。
  /// 取整到 1% 是为了少触发重排：速率每秒一帧、球腔净高 90px，
  /// 1% 已经细于一个像素。
  function setLevel(ratio) {
    const pct = Math.round(Math.max(0, Math.min(1, ratio)) * 100);
    els.orb.style.setProperty('--level', `${pct}%`);
  }
  ...
  function paintStatus(snap) {
    ...
    // 球上没有状态点：球只有 92px，里面已经是下载/上传两个读数，状态点无论放哪
    // 都要跟数据抢位置，而它一个字节都不代表。状态的正式位置是这里 ——
    // 面板头上这颗点（球上的那颗已按用户要求删掉，回归里钉着不许复活）。
    // 球上不是完全没有状态：整个球的 title 承担一句话的版本（暂停 / 断开时改写，
    // 见下方与 pipe-status 那一路）。
