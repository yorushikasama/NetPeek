...
// - 能量球形态：窗口 112×112（球 92×92，四周 10px 留给投影），球内竖排
//   「状态核心 + 下载 + 上传」，球腔自下而上注入能量液面显示下载占当前速率档位的
//   比例，点击展开为迷你窗。取舍见下方 ORB_W/ORB_H 的注释。
...
/// 能量球形态的窗口尺寸：球体 92×92（正圆，宽高必须相等），四周各留 10px 给投影
/// （`0 2px 8px` 的偏移 + 扩散 = 10，正好不被透明窗口的硬边界裁出直线）。
///
/// 中途曾改成 176×38 的胶囊横条，理由是排版预算（横排能排的字符是圆的两倍）。
/// 那一版被推翻：预算换来的是形状本身的消失 —— 一个宽高比 4.6:1 的圆角矩形
/// 读作椭圆，而「能量球」这个概念的载体首先是**圆**。现在回到正圆，排版预算
/// 用竖排两行（下载 15px 在上、上传 11px 在下）解决，而不是把球拉长。
///
/// 宽高相等是硬约束：任何一边被单独改动，CSS 里 50% 的圆角就会渲染成椭圆。
/// 改这两个常量必须连 mini.css 的 .orb 尺寸一起改，否则球不再居中。
const ORB_PAD: f64 = 10.0;
const ORB_SIZE: f64 = 92.0;
const ORB_W: f64 = ORB_SIZE + ORB_PAD * 2.0;
const ORB_H: f64 = ORB_SIZE + ORB_PAD * 2.0;
