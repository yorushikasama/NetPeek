...
## 提交前的两条留档线（2026-09-16 立）

`docs/`（正式文档：`开发进度.md` 加编号小节、`功能清单.md` 改对应屏的条目）与
`.workbuddy/memory/`（工作区记录）是**两条独立的线，交付前两条都要落**。
2026-09-16 差一点只写了 memory 就提交 —— `docs/开发进度.md` 的 §49 是先写文档再提交的，
§50（历史屏三处入口）当时只在 memory 里，提交前才发现并补上。
判定：改完一个用户可见的行为，问一句「`功能清单.md` 里那一段还准吗 / `开发进度.md` 该不该加编号节」。

提交消息照仓库既有风格：**中文、单行长主题（可带一段正文）**，写清「用户提出的现象 → 根因 →
改法 → 验收数字」。同一批未提交的改动**合成一个提交**（分开提会把共享文件的 diff 切碎）。
